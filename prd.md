...
| 14 | Script-as-orchestrator | Replace the SKILL.md prompt-based orchestrator with `plet_orchestrator.py` — a deterministic Python script that manages session lifecycle, dependency graph, retry logic, and the main loop. Eliminates compaction risk. See PLAN_RW. |
