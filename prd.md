...
| IMP_8 | **Lifecycle ownership (handoffs vs decisions):** The orchestrator sets lifecycle → `implementing` before spawning the implement subagent (reservation). The implement subagent sets lifecycle → `verifying` on completion (handoff signal). The verify subagent does NOT touch lifecycle — it only sets `lastVerdict`. The orchestrator sets all post-verify lifecycle transitions: `complete` (after merge), `queued` (retry), `blocked` (exhausted/blocked verdict). Gate scripts enforce this model. | P0 |
