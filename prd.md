...
| SF_26 | **Worktree state invariants:** During an iteration, per-iteration state files exist in two copies — the global copy (on the workstream branch) and the worktree copy (on the iteration branch). The worktree copy is authoritative during the iteration. The orchestrator writes ZERO per-iteration state during the iteration — the subagent is the sole writer. The orchestrator writes final lifecycle to the global copy ONLY after the iteration is done (verdict handoff). Global state (state.json) lives in the global copy only. No concurrent writes to the same state file — worktree during iteration, global between iterations. | P0 |
| SF_27 | **Verdict handoff:** After reading the verdict from the worktree copy, the orchestrator writes the final lifecycle to the global copy: `complete` (after merge-squash), `queued` (retry), or `blocked` (exhausted/blocked verdict). This write is committed immediately before the next `eligible()` call. | P0 |
...
| IMP_8 | **Lifecycle ownership (handoffs vs decisions):** The implement subagent sets lifecycle → `implementing` on start and `verifying` on completion (handoff signal). The verify subagent does NOT touch lifecycle — it only sets `lastVerdict`. The orchestrator sets all post-verify lifecycle transitions: `complete` (after merge), `queued` (retry), `blocked` (exhausted/blocked verdict). The orchestrator does NOT write per-iteration state during the iteration — see SF_26. Gate scripts enforce this model. | P0 |
...
| VF_14 | If all criteria pass verification, the verify agent sets `lastVerdict: "passed"`. The orchestrator then merges to workstream and sets lifecycle → `complete` (iteration frozen). The verify agent does NOT set lifecycle directly — see IMP_8 lifecycle ownership. | P0 |
...
| VF_16 | If issues are found that cannot be fixed in this context: add new criteria set to `fail`, write failing tests (red step) for each test-expressible issue as a concrete handoff to the next implementation agent, set `lastVerdict: "rejected"`, document in emergent.md and learnings.md. The branch is left with intentionally failing tests — an explicit exception to the "all tests must pass" rule. For issues that aren't test-expressible (e.g., architectural concerns), document why no red test was created. The orchestrator reads `lastVerdict` and sets lifecycle → `queued` (retry) or `blocked` (retry exhausted) — the verify agent does NOT set lifecycle. | P0 |
...
| RT_4 | `plet/trace/` contains two trace files per phase per iteration: `{iteration_id}-{phase}-{attempt}-transcript.ndjson` (raw I/O in Claude Code's native JSONL format, captured by `plet_invoke.py` from subprocess output) and `{iteration_id}-{phase}-{attempt}-events.ndjson` (semantic events, written by subagent via `plet_trace.py`). Semantic event schema defined in `references/state-schema.md`. | P0 |
...
**CLI tools** (agent-callable, listed in `allowed-tools`):
...
**Utility modules** (imported by scripts, not CLI tools):

| Module | Purpose |
|--------|---------|
| `util_cli.py` | Argument parsing, validation, dispatch, shared CLI helpers |
| `util_io.py` | Atomic file I/O, path derivation, convenience loaders |
| `util_id.py` | Plet ID generation (Crockford Base32) |
| `util_state.py` | State file validation and validated loading |
| `util_subprocess.py` | Subprocess execution with capture |
...
Orchestrator
    ...
    ├── plet_invoke.py run         → launch claude -p subprocess
...
|-----------|-----------|-------|
| Orchestrator | `plet_invoke.py` → `claude -p` subprocess | Spawns subagents via subprocess for reliable transcript capture |
...
Plet uses two testing strategies: (1) **automated tests** for enforcement scripts (1230+ tests across 16 files, stdlib-only, red/green discipline), and (2) **eval-based testing** for the skill's prompt-driven behavior (case studies, comparison runs, per-role evaluation).
