...
1. **Bootstrap check** — if no NOTES.md exists, run § Bootstrap first
2. **Status** — report file size, last modified, section summary
3. **Catch-up scan** — scan the current conversation for decisions, preferences, or rejected alternatives that aren't yet in NOTES.md
4. **Reorg check** — if drift signals are present (see § Reorganization), flag them
5. **Prompt** — present uncaptured items (and reorg suggestions if any) and offer to act on them

### Subcommand Overrides

Auto-detect handles most invocations. These overrides skip straight to a specific mode:

- `/notes bootstrap` — create NOTES.md from scratch (even if one exists, for re-scaffolding)
- `/notes reorg` — run a § Reorganization pass
- `/notes catch-up` — skip status, go straight to conversation scan
