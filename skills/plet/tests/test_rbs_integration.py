...
def test_mock_audit_rebase_prep_conflict():
    ...
    with tempfile.TemporaryDirectory() as d:
        ...
        assert os.path.exists(os.path.join(git_dir, "rebase-merge")) or \
...
def test_integration_handle_passed_verdict_real_git():
    ...
    with tempfile.TemporaryDirectory() as d:
        ...
        try:
            sink = CaptureSink()
            completed, blocked = plet_orchestrator._handle_passed_verdict(
                "ID_001", plet_dir, sink, 0, {}
            )
            ...
            assert os.path.exists(os.path.join(repo, "ID_001_file.txt")), \
                "Iteration file not on workstream"
...
def test_state_json_divergence_lifecycle_update():
    ...
    with tempfile.TemporaryDirectory() as d:
        repo, plet_dir, ws = setup_project(d,
            iter_ids=["ID_001", "ID_002"],
            lifecycles={"ID_001": "verifying", "ID_002": "implementing"})
        ...
        assert gs["lifecycles"]["ID_002"] == "complete", \
            f"Lost lifecycle update: {gs['lifecycles']}"
...
def test_r11_scenario_state_survives_requeue():
    ...
    with tempfile.TemporaryDirectory() as d:
        repo, plet_dir, ws = setup_project(d,
            iter_ids=["ID_001", "ID_002"],
            lifecycles={"ID_001": "verifying", "ID_002": "verifying"})
        ...
        plet_state = state_json_path(plet_dir)
        ...
        for _ in range(5):
            r = sp.run(["git", "-C", repo, "rebase", "--continue"],
        ...
        assert final_state["attempts"]["implement"] == 2, \
            f"Expected attempt 2, got: {final_state['attempts']}"
...
def test_full_requeue_cycle_real_git():
    ...
    with tempfile.TemporaryDirectory() as d:
        ...
        porcelain, _, _ = git(repo, "status", "--porcelain")
        conflict_files = [l[3:].strip() for l in porcelain.split("\n") if l.startswith(("UU", "AA"))]
