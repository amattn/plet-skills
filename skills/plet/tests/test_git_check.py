...
def write_iter_state(plet_dir, iter_id="ITR_001", **overrides):
...
def setup_clean_iteration(d):
    ...
    write_global_state(plet_dir, lifecycles={"ITR_001": "implementing"})
...
def test_cki_all_pass():
    ...
    with tempfile.TemporaryDirectory() as d:
        ...
        stdout, _, _ = run(["check-iteration", plet_dir, "--iter-id", "ITR_001", "--phase", "implement"], cwd=repo)
...
def test_cki_wrong_branch():
    ...
    with tempfile.TemporaryDirectory() as d:
        ...
        stdout, _, _ = run(
            ["check-iteration", plet_dir, "--iter-id", "ITR_001", "--phase", "implement"], expect_exit=1, cwd=repo
        )
...
def test_cki_merge_commit():
    ...
    with tempfile.TemporaryDirectory() as d:
        ...
        stdout, _, _ = run(
            ["check-iteration", plet_dir, "--iter-id", "ITR_001", "--phase", "implement"], expect_exit=0, cwd=repo
        )
...
def test_cki_stashes_warn():
    ...
    with tempfile.TemporaryDirectory() as d:
        ...
        stdout, _, _ = run(
            ["check-iteration", plet_dir, "--iter-id", "ITR_001", "--phase", "implement"], expect_exit=2, cwd=repo
        )
...
def test_cki_not_git_repo():
    ...
    with tempfile.TemporaryDirectory() as d:
        ...
        _, stderr, _ = run(
            ["check-iteration", plet_dir, "--iter-id", "ITR_001", "--phase", "implement"], expect_exit=1, cwd=d
        )
...
def test_cki_invalid_phase():
    ...
    with tempfile.TemporaryDirectory() as d:
        ...
        _, stderr, _ = run(
            ["check-iteration", plet_dir, "--iter-id", "ITR_001", "--phase", "plan"], expect_exit=1, cwd=repo
        )
...
def test_cki_dry_run_rejected():
    ...
    with tempfile.TemporaryDirectory() as d:
        ...
        _, stderr, _ = run(
            ["check-iteration", plet_dir, "--iter-id", "ITR_001", "--phase", "implement", "--dry-run"],
            expect_exit=1,
            cwd=repo,
        )
...
def test_cki_read_only():
    ...
    with tempfile.TemporaryDirectory() as d:
        ...
        run(["check-iteration", plet_dir, "--iter-id", "ITR_001", "--phase", "implement"], cwd=repo)
...
def test_cks_orphaned_worktree():
    ...
    with tempfile.TemporaryDirectory() as d:
        ...
        write_global_state(plet_dir, lifecycles={"ITR_001": "withdrawn"})
        write_iter_state(plet_dir, iter_id="ITR_001")
        git_run(repo, ["add", "plet/"])
        ...
        wt_path = os.path.join(repo, ".plet", "worktrees", "LOGA", "ITR_001")
        os.makedirs(os.path.dirname(wt_path), exist_ok=True)
        git_run(repo, ["worktree", "add", wt_path, "plet/LOGA/loop1/ITR_001"])
...
def test_cks_unmerged_complete():
    ...
    with tempfile.TemporaryDirectory() as d:
        repo, plet_dir = setup_session(d, num_iters=1, complete_ids=["ITR_001"])

        # ITR_001 is complete but NOT merged to workstream
...
def test_cks_workstream_missing_no_active():
    ...
    with tempfile.TemporaryDirectory() as d:
        ...
        write_global_state(plet_dir, lifecycles={"ITR_001": "ineligible"})
        os.makedirs(state_dir_path(plet_dir), exist_ok=True)
        write_iter_state(plet_dir, iter_id="ITR_001")
        git_run(repo, ["add", "plet/"])
        git_run(repo, ["commit", "-m", "add plet"])
...
def test_cks_workstream_missing_with_active():
    ...
    with tempfile.TemporaryDirectory() as d:
        ...
        write_global_state(plet_dir, lifecycles={"ITR_001": "queued"})
        os.makedirs(state_dir_path(plet_dir), exist_ok=True)
        write_iter_state(plet_dir, iter_id="ITR_001")
        git_run(repo, ["add", "plet/"])
        git_run(repo, ["commit", "-m", "add plet"])
...
def test_cks_orphaned_branch():
    ...
    with tempfile.TemporaryDirectory() as d:
        ...
        git_run(repo, ["branch", "plet/LOGA/loop1/ITR_999"])

        stdout, _, _ = run(["check-session", plet_dir], expect_exit=2, cwd=repo)
        check("orphaned-branches warned", "orphaned-branches" in stdout and "WARN" in stdout)
        check("ITR_999 mentioned", "ITR_999" in stdout)
