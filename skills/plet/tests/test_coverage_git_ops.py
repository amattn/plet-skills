...
def test_derive_iteration_branch():
    ...
    check("iteration branch", git_ops.derive_iteration_branch(gs, ist) == "plet/LOGA/loop3/ID_005")
...
def test_execute_audit_tag_create():
    ...
    try:
        ...
        try:
            ...
            check("tag created", git_ops.tag_exists("plet/TEST/loop1/audit/ID_001/implement-1", cwd=d))
        finally:
            os.chdir(old_cwd)
    finally:
        shutil.rmtree(d)
...
def test_execute_audit_tag_dry_run():
    ...
    try:
        ...
        try:
            ...
            check("dry run no tag", not git_ops.tag_exists("plet/TEST/loop1/audit/ID_001/implement-1", cwd=d))
        finally:
            os.chdir(old_cwd)
    finally:
        shutil.rmtree(d)
...
def test_cmd_audit_tag_invalid_phase():
    ...
    try:
        rc = exit_code(git_ops.cmd_audit_tag([plet_dir, "--iter-id", "ID_001", "--phase", "invalid"]))
...
def test_cmd_audit_tag_integration():
    ...
    try:
        ...
        os.chdir(d)
        try:
            rc = exit_code(git_ops.cmd_audit_tag([plet_dir, "--iter-id", "ID_001", "--phase", "implement"]))
