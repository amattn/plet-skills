...
| ID | Requirement | Priority |
|----|-------------|----------|
| INV_RUN_CMD_1 | Usage: `plet_CLIWRAPPER.py run <plet_dir> --iter-id ITR_xxx --phase implement|verify --cwd <worktree_path> [--max-budget N] [--model MODEL] [--permission-mode MODE] [--dry-run] [--output json [--pretty] [--fields f1,f2]]` | P0 |
...
2. Calls: `invoke.py run plet/ --iter-id ITR_001 --phase implement --cwd .plet/worktrees/TEST/ITR_001`
...
2. `invoke.py run plet/ --iter-id ITR_001 --phase implement --cwd /tmp/wt --dry-run`
...
```bash
invoke.py run plet/ --iter-id ITR_001 --phase implement \
