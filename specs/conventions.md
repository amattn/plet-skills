...
| UNV_CMD_31 | **JSON-first output:** The JSON structure is the single source of truth for command output. Plain text mode takes the JSON object and formats it for human readability — it must not compute output independently. Pattern: (1) build the JSON result dict, (2) pass it to a text formatter. The formatter is pure presentation — no business logic, no additional queries, no fields absent from the JSON. This prevents drift between text and JSON output paths. | P0 |
...
| UNV_CMD_29 | **Unknown flags error.** Each command defines its set of valid flags. If a flag is passed that isn't in the valid set (and isn't a universal flag like `--output`, `--pretty`, `--fields`, `--no-log`), error: `Error: unknown flag --{flag}. Run: script command --help`. Agent-first: fail loudly on typos (e.g., `--itr-id` instead of `--iter-id`) rather than silently ignoring. Implementation: per-command validation after `parse_kwargs`, likely extracted into a `util_cli` helper after the first implementations prove the pattern. Retrofit existing scripts after new scripts validate the approach. | P0 |
| UNV_CMD_28 | **`--no-log` flag:** All scripts accept `--no-log` (handled by `dispatch()`), which suppresses invocation logging and cascades to child processes via `PLET_NO_LOG=1` env var. Intentionally excluded from `--help` output — this flag is for tests and GUIs only, not agent use. Scripts that call other scripts internally use the `no_log_commands` parameter in `dispatch()` to prevent recursive logging. | P0 |
