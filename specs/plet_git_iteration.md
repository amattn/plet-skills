...
|----|-------------|----------|
| GTI_PUR_1 | Branch naming convention enforcement. Agents call this instead of constructing branch names, eliminating naming drift across iterations. | P0 |
| GTI_PUR_2 | Worktree lifecycle management. Each iteration gets an isolated working directory — eliminates stashing (FB_30) and prevents cross-branch contamination (FB_35). | P0 |
| GTI_PUR_3 | Enforces the branch/tag conventions defined in `prd.md` § Branch and tag conventions. | P0 |
...
**Properties:** mutating (creates git worktree and branch), not idempotent (errors if worktree exists), atomic (worktree either fully created or not)
...
| GTI_WTC_INP_4 | `--worktree-dir` — (optional) parent directory for worktrees. Default: `.plet/worktrees/`. The iteration worktree is created at `{worktree-dir}/{iter_id}/`. | P1 |

#### Outputs (GTI_WTC_OUT)

| ID | Requirement | Priority |
|----|-------------|----------|
| GTI_WTC_OUT_1 | Text mode: `OK — created worktree at {path} on branch {branch}` to stdout, exit 0 | P0 |
| GTI_WTC_OUT_2 | JSON mode: `{"status":"ok", "command":"worktree-create", "worktreePath":"...", "branchName":"...", "baseBranch":"...", "iterationId":"...", ...}` | P0 |
...
| GTI_WTC_PRE_2 | `state_json` exists and contains valid JSON with `projectId` and `loopSessionCount` | P0 |
...
| GTI_WTC_PRE_5 | Branch name does not already exist (error if it does — prevents accidental reuse) | P0 |
...
| ID | Requirement | Priority |
|----|-------------|----------|
| GTI_WTC_PST_1 | Worktree directory exists at `{worktree-dir}/{iter_id}/` | P0 |
...
| GTI_WTC_BHV_3 | Creates worktree with new branch in one git operation: `git worktree add -b {branch} {path} {base}` | P0 |
| GTI_WTC_BHV_4 | Creates the worktree parent directory if it doesn't exist | P0 |
| GTI_WTC_BHV_5 | Worktree path convention: `{worktree-dir}/{iter_id}/` — iteration ID as directory name for easy identification | P0 |
...
| GTI_WTR_PRE_2 | `state_json` exists and contains valid JSON with `projectId` and `loopSessionCount` | P0 |
...
| ID | Requirement | Priority |
|----|-------------|----------|
| GTI_WTR_BHV_1 | Derives worktree path from state.json + iter-id: `{worktree-dir}/{iter_id}/` | P0 |
...
| GTI_EDG_2 | Branch already exists on `worktree-create` — error (prevents accidental reuse of a branch from a previous attempt) | P0 |
...
| GTI_EDG_7 | `state.json` missing `projectId` — error with specific field name | P0 |
| GTI_EDG_8 | `state.json` missing `loopSessionCount` — error with specific field name | P0 |
...
| GTI_ERR_5 | `state.json` missing required field → `Error: state.json missing required field '{field}'` | P0 |
| GTI_ERR_6 | `projectId` invalid format → `Error: projectId '{value}' does not match pattern [A-Z][A-Z0-9]{2,5}` | P0 |
| GTI_ERR_7 | Worktree path already exists → `Error: worktree path already exists: {path}. Remove with worktree-remove first.` | P0 |
| GTI_ERR_8 | Branch already exists → `Error: branch already exists: {branch}. This may be from a previous attempt.` | P0 |
...
| ID | Requirement | Priority |
|----|-------------|----------|
| GTI_FMT_1 | Reads `plet/state.json` for `projectId`, `loopSessionCount`, `refineSessionCount` | P0 |
| GTI_FMT_2 | Branch naming convention per `prd.md`: `plet/{projectId}/loop{N}/{iter_id}`, `plet/{projectId}/loop{N}/workstream`, `plet/{projectId}/refine{N}/workstream` | P0 |
| GTI_FMT_3 | Worktree path convention: `{worktree-dir}/{iter_id}/` (default worktree-dir: `.plet/worktrees/`) | P0 |
...
2. `plet_git_iteration.py worktree-create plet/state.json --iter-id ID_001`
3. Script creates worktree at `.plet/worktrees/ID_001/` on branch `plet/LOGA/loop1/ID_001`
...
3. For each orphaned worktree: `plet_git_iteration.py worktree-remove plet/state.json --iter-id {id}`
4. Clean state for new session

### GTI_AFL_4: Subagent verifies its branch context

1. Implement subagent starts in a worktree
2. `plet_git_iteration.py branch-name plet/state.json --iter-id ID_001`
...
plet_git_iteration.py branch-name plet/state.json --iter-id ID_003 --output json
...
# OK — created worktree at .plet/worktrees/ID_001/ on branch plet/LOGA/loop1/ID_001

# Verify
ls .plet/worktrees/ID_001/
# (project files visible in isolated worktree)

git worktree list
# /path/to/project                  abc1234 [main]
# /path/to/project/.plet/worktrees/ID_001  def5678 [plet/LOGA/loop1/ID_001]

# Remove (keep branch for rebase/merge)
plet_git_iteration.py worktree-remove plet/state.json --iter-id ID_001
# OK — removed worktree at .plet/worktrees/ID_001/
...
# OK — removed worktree at .plet/worktrees/ID_001/ and branch plet/LOGA/loop1/ID_001
```

### GTI_EXM_3: Dry-run

```bash
plet_git_iteration.py worktree-create plet/state.json --iter-id ID_005 --dry-run
# DRY RUN — would create worktree at .plet/worktrees/ID_005/ on branch plet/LOGA/loop1/ID_005
```
...
| GTI_DEP_2 | imports | `util_io` | `load_json` |
...
| ID | Requirement | Priority |
|----|-------------|----------|
| GTI_NFR_1 | Git operations via `subprocess.run()` with explicit args (no shell=True) — prevents injection | P0 |
...
| GTI_DXP_3 | `branch-name` text output is bare (no "OK —" prefix) for shell capture: `BRANCH=$(plet_git_iteration.py branch-name ...)` | P0 |
...
| # | Question | Context |
|---|----------|---------|
| 1 | Should `.plet/worktrees/` be added to `.gitignore` automatically? | Worktrees are local working state, not committed. The script could append to `.gitignore` on first use, or document that the user should add it. |
