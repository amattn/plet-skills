...
> - **Sequential merge-squash:** Merge-squash must be serial (one iteration at a time). Parallel iterations all append to shared runtime artifacts (progress.md, learnings.md, emergent.md) — concurrent merge would produce conflicts. Iterations execute in parallel; only merging is serial. (from GTO RQ_7, NOTES.md § Worktree + shared artifacts merge strategy)
