...
Autonomous agents accumulate tech debt iteration by iteration — each subagent optimizes locally for its acceptance criteria without seeing the broader codebase trajectory. LOGA Run 6 demonstrated this: main.go grew to 433 lines with repetitive subcommand handling across 13 iterations.

**Core idea:** Add a refactor→verify loop at milestone boundaries (and possibly every iteration). The refactor agent analyzes the codebase for structural issues (duplication, excessive file length, missing abstractions) and produces cleanup changes. A verify agent confirms the refactoring didn't break anything.

**Open questions:**
- Should refactoring happen at every iteration or only at milestone boundaries?
- Does the refactor agent get its own reference file (like implement.md)?
- How does a refactor iteration define acceptance criteria? ("Code is cleaner" is not verifiable.)
- Should refactor iterations be auto-generated by the orchestrator or proposed during refine?
- What's the relationship between Tier 1 refactoring (within-iteration, handled by verify's fix-in-place) and Tier 2 (cross-iteration, this feature)?

**Depends on:** FOO_70, NOTES.md § Two-tier refactoring model, NOTES.md § Refactor step or phase in the loop.
...
| COV_15 | Injected script runner for orchestrator | Replace hardcoded `_run_script`/`_run_script_json` subprocess calls with injectable runner. Tests provide mock that calls cmd_* functions directly. Covers streaming loop, conflict recovery, all decision logic. Orchestrator 58% → 90%+. |
