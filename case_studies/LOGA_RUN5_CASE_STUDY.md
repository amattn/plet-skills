...
1. **Env var injection (seq 44).** Subagent found scripts immediately via `$PLET_SCRIPTS_DIR`. No searching. Run 4's 8-minute script search is gone.
2. **IST scripts called correctly.** start-phase, update-activity, update-criterion, set-verdict all used properly.
3. **100% verify first-pass rate.** All 3 iterations passed verification on the first attempt. The verify subagent does quality work.
4. **Bootstrap permissions detection.** Agent correctly told user to configure bypassPermissions.
5. **Compact progress entries from dispatch auto-logger.** One-liners with trace IDs instead of 15-line entries.
6. **Code quality.** 681 lines of Go, 35 tests, proper package structure. Would pass human code review.

### What Didn't Work Well

1. **Dependency promotion missing.** After ID_001 completed, ID_002 stayed `ineligible`. Orchestrator stopped. Required manual intervention + code fix.
2. **State.json merge conflict on ID_002.** Workstream and worktree had different session timestamps. Merge-squash failed with conflict markers.
3. **Plan committed to main.** Third consecutive run where the agent ignores plan branch instructions.
4. **Allow pattern doesn't match.** `Bash(plet_*.py*)` in settings.json doesn't match `$SCRIPTS/plet_fingerprint.py ...` invocations.
5. **progress.md is 12,975 lines.** Mostly auto-logged invocations. Subagent-written entries are a small fraction.
6. **Two loop sessions needed.** Orchestrator stopped after ID_001 (no promotion), had to be re-launched.

### Bugs Found (fixed during/after run)

| Bug | Fix | Status |
|-----|-----|--------|
| Dependency promotion missing | `_promote_eligible()` in orchestrator | Committed, not in published 0.4.1 |
| State.json merge conflict | `.gitattributes merge=ours` | Committed, not in published 0.4.1 |
| Invoke permission mode default | Auto-detect from settings.json | Committed, not in published 0.4.1 |
| Progress entries dump full prompt | Clipped to metadata + trace ref | Committed, not in published 0.4.1 |
| Files changed always "(none)" | Removed field entirely | Committed, not in published 0.4.1 |

### Open Questions

1. Should progress.md auto-logging from dispatch be reduced further? 12,975 lines for 3 iterations is excessive.
2. ID_002 worktree not cleaned up (still exists). Is worktree cleanup working?
3. Plan branch creation needs enforcement, not prose — three runs in a row it's ignored.
...
13. **"Files changed" mostly useless.** 19 entries with the field: 9 say "(none)" (auto-logged), 6 are template examples from embedded reference docs, 4 have real files (subagent-written). Removed the field entirely.
